...
function CemberinAlani(yariCap, piNum) {
...
// 3b çözümü:
ucetambolunenler = [];
sayilar.forEach((num) => (num % 3 === 0 ? ucetambolunenler.push(num) : true));
...
let objArray = sayilar.flatMap((item) => {
  return [
    {
      sayi: item,
      tekrarsayisi: 1,
...
let countObject = objArray.reduce((acc, item) => {
  acc[item.sayi] = acc[item.sayi]
    ? acc[item.sayi] + item.tekrarsayisi
    : item.tekrarsayisi;
  return acc;
}, {});
tekraredensayilar = [];
for (let i = 0; i < Object.keys(countObject).length; i++) {
  tekraredensayilar.push(
    `${Object.keys(countObject)[i]} sayısı ${
      Object.values(countObject)[i]
